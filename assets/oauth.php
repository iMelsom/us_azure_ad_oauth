...
$sessionData = $modDB->QuerySingle('SELECT * FROM aao_tblAuthSessions WHERE txtSessionKey=\'' . $modDB->Escape($_SESSION['sessionkey']) . '\'');
...
if ($sessionData) {
    ...
    $idToken = base64_decode(str_replace("_", "/", str_replace("-", "+", explode('.', $reply->id_token)[1])));
    $modDB->Update('aao_tblAuthSessions', array('txtToken' => $reply->access_token, 'txtRefreshToken' => $reply->refresh_token, 'txtIDToken' => $idToken, 'txtRedir' => '', 'dtExpires' => date('Y-m-d H:i:s', strtotime('+' . $reply->expires_in . ' seconds'))), array('intAuthID' => $sessionData['intAuthID']));
