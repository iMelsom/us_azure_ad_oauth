...
class modAuth {
	...
	if (session_status() == PHP_SESSION_NONE) session_start();
        ...
        if (isset($_SESSION['sessionkey'])) {
            ...
            $res = $this->modDB->QuerySingle('SELECT * FROM aao_tblAuthSessions WHERE txtSessionKey = \'' . $this->modDB->Escape($_SESSION['sessionkey']) . '\'');
            ...
            if ($_GET['action'] == 'logout') {
                ...
                $this->modDB->Delete('aao_tblAuthSessions', array('intAuthID' => $res['intAuthID']));
                unset($_SESSION['sessionkey']);
                session_destroy();
	            ...
	            $response = $this->oAuth->postRequest('token', $oauthRequest);
                    ...
                    if ($reply->error) {
                        if(substr($reply->error_description, 0, 12) == 'AADSTS70008:') {
                            ...
                            $this->modDB->Update('aao_tblAuthSessions', array('txtRedir' => $url, 'txtRefreshToken' => '', 'dtExpires' => date('Y-m-d H:i:s', strtotime('+5 minutes'))),  array('intAuthID' => $res['intAuthID']));
		    ...
		    $this->modDB->Update('aao_tblAuthSessions', array('txtToken' => $reply->access_token, 'txtRefreshToken' => $reply->refresh_token, 'txtIDToken' => $idToken, 'txtRedir' => '', 'dtExpires' => date('Y-m-d H:i:s', strtotime('+' . $reply->expires_in . ' seconds'))), array('intAuthID' => $res['intAuthID']));
	    ...
	    if (!$allowAnonymous || $_GET['login'] == '1') {
                ...
                $this->modDB->Insert('aao_tblAuthSessions', array('txtSessionKey' => $sessionKey, 'txtRedir' => $url, 'txtCodeVerifier' => $this->oAuthVerifier, 'dtExpires' => date('Y-m-d H:i:s', strtotime('+5 minutes'))));
	...
	$maxRefresh = strtotime('-72 hour');
        $this->modDB->Query('DELETE FROM aao_tblAuthSessions WHERE dtExpires < \'' . date('Y-m-d H:i:s', $maxRefresh) . '\'');
    }
